...
if (isset($_POST["submit"])) {
			...
			if ($book_name=="") {
			    ...
			    $sql = "SELECT * FROM issuebook WHERE book_name ='$book_name' AND user_email='$user_email' AND status!='returned' ";
			    $result = mysqli_query($conn,$sql);
			    $count = mysqli_num_rows($result);
				    if ($count == 0) {
						$sql= mysqli_query($conn,"INSERT INTO issuebook(book_id, book_name, user_email, user_id, user_name, Issue_date, return_date, status) VALUES('$bookid', '$book_name', '$user_email', '$userid','$username', '$issuedate', '$returndate','pending')");
